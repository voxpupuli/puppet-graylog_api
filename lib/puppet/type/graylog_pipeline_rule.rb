...
Puppet::Type.newtype(:graylog_pipeline_rule) do
  ...
  validate do
    match_data = self[:source].match(%r{\A\s*rule\s+"(.+?)"})
    raise('Rule source does not appear to begin with a rule-title declaration!') unless match_data

    inline_name = match_data.captures[0]
    raise("Name in rule source (#{inline_name}) doesn't match resource title (#{self[:name]})!") unless inline_name == self[:name]
  end
